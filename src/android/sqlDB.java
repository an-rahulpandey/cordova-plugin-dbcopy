...
public class sqlDB extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args,
		...
		if (action.equalsIgnoreCase("copyTo")) {
			this.copyDBTo(args.getString(0), args.getString(1), callbackContext);
			return true;
	...
	private void copyDBTo(String path, String dbName, final CallbackContext callbackContext) {
		...
		if (dbexists) {
			...
			cordova.getThreadPool().execute(new Runnable() {
				...
				public void run() {
					...
					// TODO Auto-generated method stub
					try {
						dbhelper.exportdatabase(dbpath);
						plResult = new PluginResult(PluginResult.Status.OK);
						callbackContext.sendPluginResult(plResult);
					} catch (Exception e) {
